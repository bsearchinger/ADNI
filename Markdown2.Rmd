...
We plan to evaluate our success with the following metrics:
 ...
 - First Stage Accuracy, True Positive, False Negative, & Misclassification Rates
...
5-fold crass-validation was then run for each of the four initial baseline models.  The following code performs the cross-validation and also calculates performance metrics of interest including the misclassification, true positive, and false positive rates.  Specific functions needed for the cross-validation and metrics calculations are defined in the `cv_functions.R` script.  
...
row.names(bl_summary) <- c(
        "Linear w/MRI", "Linear w/o MRI", "Logistic w/MRI", "Logistic w/o MRI")
colnames(bl_summary) <- c(
        "Misclass. Rate", "True Pos.", "False Pos.", "True Neg.", "False Neg.")
...
knitr::kable(bl_summary, caption = "Cross-Validation Results", row.names = TRUE)
```


A small subset of patients also had CSF measurements taken at other visits including 12-month and 24-month follow-ups, but these comprised of far fewer observations.
